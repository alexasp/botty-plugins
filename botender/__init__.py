...
def illhaveone(bot, event, *args):
	...
	if len(args) == 0 or args[0].lower() == 'in':
		...
		try:
			...
			if success:
				yield from bot.coro_send_message(event.conv_id, "At " + name + ". find it: " + loc_url + "", image_id=image_id)
...
def mixme(bot, event, *args):
    ...
    r_url = url + url_append + '/?apiKey=' + api_key
    logger.info(r_url)
    r = yield from aiohttp.request("get", r_url)
    r_json = yield from r.json()
